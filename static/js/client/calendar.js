...
function renderCalendarEvents(element, event, index) {
  ...
  event_details.textContent = event;
...
function careGiver(element) {
  ...
  caregiver_title.textContent = "caregiver";
...
function calendarNav(element, month_number) {
  ...
  calendar_nav.append(calendar_today);
    ...
    if (appointment[year][month]) {
      if (appointment[year][month][day]) {
        appointment[year][month][day].forEach((event, index) => {
          renderCalendarEvents(element, event, index);
        });
      }
    }
  }
